...
class GeneralUtils():
    ...
    def get_random_indices_non_uniform(self, prime_labels, test_num):
        ...
        bias_rate = 0.8
        biased_class_random_indices = list(np.random.choice(biased_class_indices, int(test_num * bias_rate), replace=False))
        other_random_indices = list(np.random.choice(other_indices, int(test_num * (1 - bias_rate)), replace=False))
